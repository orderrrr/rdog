    ...
    fn new(
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                module: &engine.shaders.get("voxel_raster").unwrap().module, // TODO actually error handle.
                entry_point: Some("vs"),
                buffers: &[],
            ...
            fragment: Some(wgpu::FragmentState {
                module: &engine.shaders.get("voxel_raster").unwrap().module,
                entry_point: Some("fs"),
                compilation_options: wgpu::PipelineCompilationOptions::default(),
...
impl PassConstruct for DiffPass {
    fn new(engine: &Engine, device: &wgpu::Device, _: &Camera, buffers: &Buffers) -> Box<dyn Pass> {
        ...
        let init = ComputePass::builder(name)
            .bind([
                &buffers.get_old("curr_camera").bind_readable(),
                &buffers.get_old("globals").bind_readable(),
                &buffers.get_old("config").bind_readable(),
            ...
            .build(
                device,
                &"main",
                &engine.shaders.get("voxel_init").unwrap().module,
            );

        Box::new(Self {
            name: name.to_string(),
            compute_passes: vec![init],
        })
    }
}
...
impl PassConstruct for AccumPass {
    fn new(engine: &Engine, device: &wgpu::Device, _: &Camera, buffers: &Buffers) -> Box<dyn Pass> {
        ...
        let voxel_step_final = ComputePass::builder(name)
            .bind([
                &buffers.get_old("curr_camera").bind_readable(),
                &buffers.get_old("config").bind_readable(),
                &buffers.get_old("render_tx").bind_writable(),
        ...
        Box::new(Self {
            name: name.to_string(),
            compute_passes: vec![voxel_step_final],
        })
    }
}
