        ...
        match e {
            RdogEvent::Recompute | RdogEvent::RecomputePasses => {
                ...
                for handle in state.cameras.values() {
                    ...
                    let draster = TracePass::new(&engine, &device, cam, &buf);
                    let raster = RasterPass::new(&engine, &device, cam, &buf);
                    let accum = OutputTracePass::new(&engine, &device, cam, &buf);

                    p.set_pass_order(vec![&draster, &raster]);
                    p.insert_all(vec![readback, draster, raster, accum]);
