#define_import_path sphere_scene
...
//    var out = vec3(vec3f(dt.x, pack_material_ids(1.0, 2.0), dt.y));
    ...
    var s = vec3f(abs(length(p - vec3f(0.0, 0.0, 0.0)) - 1.0), pack_material_ids(1.0, 1.0), 0.0);
    s = sd_min3(vec3f(length(p - vec3f(0.0, 0.0, 0.0)) - 0.2, pack_material_ids(2.0, 2.0), 0.0), s);
...
fn sample_atmos(sr: Ray) -> vec3f {
    return vec3f(
        sin(sr.o.x * 0.050 * globals.time.x + 0.00000) / 2.0 + 1.0,
        cos(sr.o.x * 0.100 * globals.time.x + 0.25808) / 2.0 + 1.0,
        sin(sr.o.y * 0.075 * globals.time.x + 0.66177) / 2.0 + 1.0
    ) * 1.0;
}
