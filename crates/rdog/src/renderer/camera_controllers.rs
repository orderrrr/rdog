...
impl RenderControllers {
    pub fn add(&self, camera: CameraController) -> CameraHandle {
        let handle = CameraHandle::new(self.next_id.fetch_add(1, std::sync::atomic::Ordering::SeqCst));
        self.cameras.write().unwrap().insert(handle, camera);
        handle
    }

    pub fn get(&self, camera_handle: CameraHandle) -> impl std::ops::Deref<Target = CameraController> + '_ {
        std::sync::RwLockReadGuard::map(self.cameras.read().unwrap(), |cameras| {
            cameras.get(&camera_handle)
    ...
    pub fn get_mut(&self, camera_handle: CameraHandle) -> impl std::ops::DerefMut<Target = CameraController> + '_ {
        std::sync::RwLockWriteGuard::map(self.cameras.write().unwrap(), |cameras| {
            cameras.get_mut(&camera_handle)
                .unwrap_or_else(|| panic!("camera does not exist: {:?}", camera_handle))
        })
    }

    pub fn iter_mut(&self) -> Vec<impl std::ops::DerefMut<Target = CameraController> + '_> {
        let handles: Vec<CameraHandle> = self.cameras.read().unwrap().keys().cloned().collect();
        handles.into_iter()
            .map(|handle| self.get_mut(handle))
            .collect()
    }
