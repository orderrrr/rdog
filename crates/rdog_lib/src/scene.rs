...
pub fn map_scene_1(scene: &Scene, posi: Vec3) -> Vec2 {
    let pos = aar(posi, vec3(0.05, 0.5, 0.1).normalize(), 1.0);

    let l = scene.lights(posi);
    let s1 = sd_round_box(pos + vec3(-1.0, -1.0, 1.0), Vec3::splat(0.5), 0.1);
    let s2 = shape(posi, scene.globals.time.x, scene.globals.seed);
    let s3 = sphere(posi + vec3(-1.0, -1.0, -1.0), 0.4);

    let p = plane(posi, vec4(0.0, 1.0, 0.0, 0.0));
    let p = vec2(p, 4.0);
    let s1 = vec2(s1, 3.0);
    let s2 = vec2(s2, 2.0);
    let s3 = vec2(s3, 5.0);
